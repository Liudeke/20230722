...
from data.experiment3 import meshData
...
class Body:
    ...
    def set_Vm(self):
        for i in self.Vm:
            self.Vm[i] = self.vertex[i][1] * 30.0 / 1000.0
...
class XPBD_SNH_with_active:
    def __init__(self, body: Body, num_pts_np: np.ndarray,
                 ...
                 dt=1. / 60., numSubsteps=1, numPosIters=10):
        self.body = body
    ...
    def cal_active_force(self):
        ...
        for i in self.elements:
            ...
            for j in ti.static(range(4)):
                for m in ti.static(range(3)):
                    self.vert_active_force[vid[j]][m] -= dpsidx[j, m] * vol * vol
...
def experiment3():
    ...
    if open_gui:
        ...
        force_field_tet = ti.Vector.field(3, float, shape=(8, ))
        elemid = 10
        vid = tm.ivec4(0, 0, 0, 0)
        for i in range(4):
            vid[i] = dynamics_sys.elements[elemid][i]
            force_field_tet[2 * i] = dynamics_sys.pos[vid[i]]
        ...
        while window.running:
            ...
            scene.lines(force_field_tet, color=(0., 0.0, 1.), width=2.0)
